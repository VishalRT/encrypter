...
static bool derive_key_iv(const std::string &password,
  ...
  if (!PKCS5_PBKDF2_HMAC(password.c_str(), static_cast<int>(password.size()),
                         ...
                         PBKDF2_ITERS, md, static_cast<int>(buf.size()),
                         buf.data())) {
    print_openssl_error();
    return false;
  }
...
static int encrypt_file_stream(const std::string &input_path,
  ...
  std::vector<unsigned char> outbuf(BUFSIZE +
...
open_directory_handle(const std::wstring &directory) {
  HANDLE dirHandle =
...
static void watch_directory(const std::string &source_path,
  ...
  if (!directory_handle_opt.has_value()) {
    std::cerr << "Failed to open directory for watching. Error: "
              << GetLastError() << "\n";
    return;
  }
  ...
  while (ReadDirectoryChangesW(
    ...
    do {
      ...
      if (notify_info->Action == FILE_ACTION_MODIFIED &&
        ...
        std::cout << "File Modified: " << to_utf8(filename_wide)
                  << ". Re-encrypting...\n";
      ...
      notify_info = reinterpret_cast<FILE_NOTIFY_INFORMATION *>(
          reinterpret_cast<std::byte *>(notify_info) +
